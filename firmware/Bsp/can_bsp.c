...
void std_can_filter_init(void)
{
	...
	fdcan_filter.FilterType = FDCAN_FILTER_RANGE;                  //滤波器类型，范围模式
	fdcan_filter.FilterConfig = FDCAN_FILTER_TO_RXFIFO0;           //过滤器0关联到FIFO0  
	fdcan_filter.FilterID1 = 0x0000;                               //32位ID
	fdcan_filter.FilterID2 = 0x0010;                               //如果FDCAN配置为传统模式的话，这里是32位掩码
	/*	范围模式：只接收仲裁段ID数据范围从 ID1 → ID2 之间的标准格式CAN数据帧，其他的会被硬件过滤*/
...
{
	canx_receive(&hfdcan1, rx_data1);
}
uint8_t rx_data2[8] = {0};
void can2_rx_callback(void)
{
	canx_receive(&hfdcan2, rx_data2);
}
uint8_t rx_data3[8] = {0};
void can3_rx_callback(void)
{
	canx_receive(&hfdcan3, rx_data3);
}
